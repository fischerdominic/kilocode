...
export function registerCodeActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("fox-code.new.explainCode", async () => {
    ...
    vscode.commands.registerCommand("fox-code.new.fixCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("fox-code.new.improveCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("fox-code.new.addToContext", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("fox-code.new.focusChatInput", async () => {
      const view = target()
      if (!(await revealTarget(view))) return
    ...
    vscode.commands.registerCommand("fox-code.new.toggleChatSearch", async () => {
      const view = target()
      if (!(await revealTarget(view))) return
